...
- The AIF adapter is deterministic and now configurable via EDN to tune the
  weights (`:g/weights`) and `:tau` bounds. The config can be explained using
  `fucodex --aif-explain` for a worked example of how G/tau are computed.
