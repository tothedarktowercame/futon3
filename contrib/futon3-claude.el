...
(defun my-futon3-claude--start-streaming (session-id)
  ...
  (let ((buf (get-buffer-create my-futon3-claude-buffer)))
    (with-current-buffer buf
      ...
      (insert (format "\n--- Session: %s ---\n" session-id)))
    (display-buffer buf)
    ;; For now, use polling since url.el doesn't do WebSocket easily
    ;; TODO: Use websocket.el package for proper streaming
    (run-with-timer 1 nil #'my-futon3-claude--poll-session session-id)))

(defun my-futon3-claude--poll-session (session-id)
  "Poll session status (temporary until WebSocket streaming)."
            ...
            (let ((status (plist-get session :status)))
              (cond
               ((member status '("running" "starting"))
                ...
                (run-with-timer 2 nil #'my-futon3-claude--poll-session session-id))
        ...
        (let ((type (plist-get event :event/type)))
          (cond
           ((string= type "assistant/delta")
            (insert (plist-get event :assistant/text)))
           ((string= type "assistant/message")
            (insert (format "\n%s\n" (plist-get event :assistant/text))))
           ...
           ((string= type "session/finished")
            (insert (format "\n--- Session %s ---\n"
                            (plist-get event :session/status))))
           (t
            (insert (format "[%s]\n" type)))))))))
