...
Cross-references:
- [[file:holes/features/futon3.devmap][futon3.devmap]] — Prototype 4 (Trail & Proof-State Journal), Prototype 5 (Workday Instrumentation)
- [[file:holes/features/futon4.devmap][futon4.devmap]] — Lab notebook architecture (fuclaude, fucodex, lab-export)
...
* Governance Hooks (Concrete Pairings)

When you apply a classic software design pattern, add a lightweight fulab
governance hook so the decision is traceable and auditable later. Pairings
are not dependencies; they are reminders to emit the right evidence.

- Adapter -> Trade-off record (why translate instead of refactor).
- Observer -> Blast radius (who gets notified, rollback scope).
- Facade -> Blast radius + Doc written (public contract and impact boundary).
- State -> Trade-off record (assumptions + invalid transitions).
