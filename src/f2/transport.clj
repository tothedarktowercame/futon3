    ...
    (case type
      ...
      :bye (do
             ...
             {:reply {:ok true :type "bye"}
              :close? true})
      :event (let [payload (:payload envelope)
                   eid (or (get payload :id)
                           (str "E-" (.substring (str (UUID/randomUUID)) 0 6)))]
               {:reply {:ok true :type "ack" :id eid}})
      :session (let [sid (or (get-in envelope [:payload :sid])
                             (str "S-" (.substring (str (UUID/randomUUID)) 0 6)))]
                 {:reply {:ok true :type "ack" :id sid}})
