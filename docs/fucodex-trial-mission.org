...
White-space scout guard: if the proof is missing `:check/aif-trace`, do one
scout pass (reprint `episode-trace`, rerun the call once), then log the gap as
"white-space" evidence and stop rather than looping.
